...
import type { SearchLogResponse, ActivityLogResponse, SearchLogRequest, PriceSearchLogResponse, PriceSearchLogRequest } from './types'
...
export const statisticsApiSlice = apiSlice.injectEndpoints({
	...
	endpoints: builder => ({
		...
		getPriceSearchLogs: builder.query<PriceSearchLogResponse, PriceSearchLogRequest | undefined>({
			query: params => ({
				url: API.statistics.priceSearch,
				method: 'GET',
				params,
			}),
			onQueryStarted: async (_arg, api) => {

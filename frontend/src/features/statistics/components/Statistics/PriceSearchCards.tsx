...
import type { PriceSearchLog } from '../../types'
...
export const PriceSearchCards: FC<Props> = ({ data, total }) => {
	const calculatedStats = useMemo(() => {
		if (!data) {
			return { avgTime: 0, noResults: 0, foundPositions: 0 }
		}
		...
		const foundPositions = data.reduce((sum, log) => sum + log.resultsCount, 0)

		return { avgTime, noResults, foundPositions }
	}, [data])
	...
	return (
		...
		>
			...
			<StatCard icon={<Inbox />} title='Найдено позиций' value={calculatedStats.foundPositions} color='#00b894' />
